...
namespace ACS.Infrastructure.Data
{
   ...
   //             GetBatchOperation().Delete(entity);
		...
		public T FindEntity(object partitionId, object RowId)
		{
			...
			return default(T);//(T)m_table.Execute(TableOperation.Retrieve<T>(pk, rk)).Result;
		}
        ...
        public T FindEntityByRowKey(object RowId)
        {
			...
			return default(T);//m_table.ExecuteQuery(tableQuery).SingleOrDefault();
        }
		...
		public IQueryable<T> QueryEntitiesByPartition(object partitionId)
		{
			...
			return null;// m_table.ExecuteQuery(tableQuery).AsQueryable<T>();
		}

        public IQueryable<T> QueryAllEntities()
        {
			return null;// m_table.ExecuteQuery(new TableQuery<T>()).AsQueryable<T>();
        }
